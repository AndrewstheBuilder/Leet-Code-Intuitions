...
- Previous solution would work with some modification. We have to reverse the way the nodes get added on every other iteration of column loop. Intuition is to start out by creating diagonals then work our way to the zig zag solution.
```python
            ...
            for c in range(C):
                if (r,c) not in seen:
                    ...
                    while (stack):
                        ...
                        seen.add((nr,nc))
                        if rev:
                            visited.appendleft(mat[nr][nc])
                        else:
                            output.append(mat[nr][nc])
                        # calcs
                        if(nr+1<R and nc-1>=0 and (nr+1, nc-1) not in seen):
                            stack.append((nr+1,nc-1))
                    if rev:
                        # enable zig zag traversal of mat
                        output.extend(visited)
                        visited = deque()
                    rev = not rev
        return output
# Time Complexity: O(m*n) while loop only runs once for unseen nodes.
# Space Complexity: O(m*n) because of stack
```
- Simulation Approach Solution:
```python
# the simulation approach of upward and downwards movement with a while loop
    def findDiagonalOrder(self, matrix: List[List[int]]) -> List[int]:
        ...
        return result
# Time Complexity: O(M*N) we are still visiting each node
# Space Complexity: O(1) no intermediate space used in algorithm. We just populate the output and thats it. Output does not count as increasing space complexity.
```
- here is my version of the space saving solution. Where we do the diagonal traversal in the while loop
```python
    def findDiagonalOrder(self, mat: List[List[int]]) -> List[int]:
        # edge case when there are no elements
        if(mat == False or mat[0] == False):
            return []

        R = len(mat)
        C = len(mat[0])
        row = col = 0
        direction=1 #1 being up and to the right, 0 being bottom and to the left
        ...
        while row<R and col<C:
            ...
            if new_row<0 or new_row==R or new_col<0 or new_col==C:
                # Find the new head of the diagonal and adjust direction
                if direction == 1:  # Moving up-right
                    if col == C - 1:  # Hit the right boundary
                        row += 1
                    else:  # Hit the top boundary
                        col += 1
                else:  # Moving down-left
                    if row == R - 1:  # Hit the bottom boundary
                        col += 1
                    else:  # Hit the left boundary
                        row += 1
                # switch direction
                direction = 1 - direction
            else:
...
- My complex solution. I had concepts like subtracting the k when there is a gap. And if the gap exceeds k_copy then we find the answer
- if ans has not been found in the loop then we add the remaining k to the last element in the arr.
- This is the iterative solution. The array is guaranteed to be sorted in strictly increasing order so no duplicates either this should signal `Binary Search`
```python
    def findKthPositive(self, arr: List[int], k: int) -> int:
        ...
        for i in range(len(arr)):
            if(arr[i]!=prev+1):
                gap=arr[i]-prev-1 #(-1 to count current arr[i])
                if(gap>=k_copy):
                    ans = prev+k_copy
                    break
                else:
...
```python
    def findKthPositive(self, arr: List[int], k: int) -> int:
        ...
        return k
```
- Intuition we are keeping track of the possible ans by incrementing k by 1 each time we encounter a number less than k. when nums[i] surpasses k k will point to the missing kth positive number.
...
```python
    def findKthPositive(self,nums,k):
        ...
        # left is right+1 or right is left-1 either one says same thing
        # we have found the gap where the kth positive integer is
        # derivation is cancel arr[right]s:
            # arr[right] +( k - ( arr[right] - (right + 1) ))
            # ( k - ( arr[right] - (right + 1) )) this gets the missing integers before right and subtracts it from k
            # then we add the remainder of above to arr[right] to get the kth missing integer
        # find the last element before gap with k missing element
        return k+right+1
```
