...
# this algorithm creates a heap with the end times.
# first time gets a room. Then subsequent sorted time intervals we check with the end time in the heap and the start time. If they overlap then a new room gets allocated.
# rooms can be swapped once a room is established in the heap but its not removed.
	# you see when we pop from the heap we push right after maintaing the length of the heap
# so the heap will contain the length of the max number of rooms allocated at the end
```